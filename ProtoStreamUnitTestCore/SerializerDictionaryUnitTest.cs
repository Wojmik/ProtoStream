...
{
	[TestClass]
	public class SerializerDictionaryUnitTest
	{
		[DataTestMethod]
		...
		static SampleData<Data1> CreateSample1()
		{
			...
			sampleData.SampleObject = new Data1() { Int1=300, Strings111=new Dictionary<string, List<string>>(Nest1Size), };

			for(int i=0; i<Nest1Size; i++)
			{
				key=$"Pl_{i}";
				sampleData.SampleObject.Strings111.Add(key, new List<string>(Nest2Size));
				for(int j=0; j<Nest2Size; j++)
					sampleData.SampleObject.Strings111[key].Add($"{i}*{j}");
			}
		...
		public class Data1
		{
			...
			[DataMember(Order = 9)]
			public Dictionary<string, List<string>> Strings111 { get; set; }
			...
			public static void AreEqual(Data1 expected, Data1 actual)
			{
				...
				if(expected.Strings111!=null && actual.Strings111!=null)
					if(expected.Strings111.Count==actual.Strings111.Count)
						using(var enmExpected = expected.Strings111.GetEnumerator())
						using(var enmActual = actual.Strings111.GetEnumerator())
						{
							while(enmExpected.MoveNext() && enmActual.MoveNext())
							{
								Assert.AreEqual(expected: enmExpected.Current.Key, actual: enmActual.Current.Key);

								if(enmExpected.Current.Value!=null && enmActual.Current.Value!=null)
									if(enmExpected.Current.Value.Count==enmActual.Current.Value.Count)
										for(int j = 0; j<enmExpected.Current.Value.Count; j++)
											Assert.AreEqual(expected: enmExpected.Current.Value[j], actual: enmActual.Current.Value[j]);
									else
										Assert.Fail("Diferent list sizes");
								else
									Assert.AreEqual(enmExpected.Current.Value!=null, enmActual.Current.Value!=null);
